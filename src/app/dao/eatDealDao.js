...
async function getEatDeal(location, page, limit) {
    ...
    const getEatDealQuery = `
        ...
        where SUBSTRING_INDEX(SUBSTRING_INDEX(Restaurant.restaurantLocation, " ", 2), " ", -1) in (?)
          and EatDeal.status = 1 limit ?,?;
    `;
...
async function getEatDealImg(eatDealId) {
    ...
    const getEatDealImgQuery = `
        ...
        select eatDealId, imgId, eatDealImgUrl
        from EatDealImg
        where eatDealId = ?;
               ...
               round(6371 *
                     acos(cos(radians(@userLatitude)) * cos(radians(restaurantLatitude)) *
                          cos(radians(restaurantLongitude)
                              - radians(@userLongtitude)) +
                          sin(radians(@userLatitude)) * sin(radians(restaurantLatitude))), 2)
            AS 'distanceFromUser'
        ...
        select TopList.topListId,
               ...
               CASE
                   WHEN TIMESTAMPDIFF(HOUR, TopList.updatedAt, now()) > 23
                       THEN IF(TIMESTAMPDIFF(DAY, TopList.updatedAt, now()) > 7,
                               date_format(TopList.updatedAt, '%Y-%m-%d'),
...
async function postTopListBookMark(topListParams) {
    ...
    const postTopListBookMarkQuery = `
        ...
        insert into TopListBookMark (topListId, userId, status)
        values (?, ?, 1);
...
async function getTopListDetailRestaurant(topListParams2) {
    ...
    const getTopListDetailRestaurantQuery = `
        ...
        where TopListContents.topListId = ? limit ?,?;
    `;
    ...
    const otherTopListQuery = `
        ...
        from TopList
                 ...
                 left outer join (select topListId, count(*) as userBookMark
                                  ...
                                  group by topListId) UserBookMark
                                 on TopList.topListId = UserBookMark.topListId
        order by updatedAt asc limit 6;
