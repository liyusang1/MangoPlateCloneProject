...
async function allRestaurantInfo(showAllRestaurantInfoParams) {
    ...
    const allRestaurantInfoQuery = `
        select Restaurant.restaurantId,
               ...
               round(6371 *
                     acos(cos(radians(@userLatitude)) * cos(radians(restaurantLatitude)) *
                          cos(radians(restaurantLongitude)
                              - radians(@userLongitude)) +
                          sin(radians(@userLatitude)) * sin(radians(restaurantLatitude))), 2) AS 'distanceFromUser',
        ...
        where Restaurant.status = 1
          ...
          and SUBSTRING_INDEX(SUBSTRING_INDEX(restaurantLocation, " ", 2), " ", -1) in (?)
...
async function allRestaurantInfoLike(showAllRestaurantInfoParams) {
    ...
    const allRestaurantInfoQuery = `
        ...
        from Restaurant
                 ...
                 inner join (select restaurantId, count(*) as isLike
                             from RestaurantLike
                             where userId = ?
                               and status = 1
                             group by restaurantId) IsLike
...
async function allRestaurantInfoByMenuFilter(showAllRestaurantInfoParams) {
    ...
    const allRestaurantInfoQuery = `
        ...
        where SUBSTRING_INDEX(SUBSTRING_INDEX(restaurantLocation, " ", 2), " ", -1) in (?)
...
async function insertRestaurant(insertRestaurantParams) {
    ...
    const insertRestaurantQuery = `
        INSERT INTO Restaurant (restaurantName, restaurantLocation, restaurantLatitude, restaurantLongitude,
                                restaurantPhoneNumber, restaurantFilter, restaurantEnrollUserId)
        VALUES (?, ?, ?, ?, ?, ?, ?)
    `;
        ...
        select Review.reviewId,
               ...
               CASE
                   WHEN TIMESTAMPDIFF(HOUR, Review.updatedAt, now()) > 23
                       THEN IF(TIMESTAMPDIFF(DAY, Review.updatedAt, now()) > 7,
                               date_format(Review.updatedAt, '%Y-%m-%d'),
        ...
        select Restaurant.restaurantId,
               ...
               round(6371 *
                     acos(cos(radians(@currentRestaurantLatitude)) * cos(radians(restaurantLatitude)) *
                          ...
                          sin(radians(@currentRestaurantLatitude)) * sin(radians(restaurantLatitude))),
                     2) AS 'distanceFromRestaurant',
...
async function getCurrentArea() {
    ...
    const getCurrentAreaQuery = `
        ...
        select areaName,
               round(6371 *
                     acos(cos(radians(@currentRestaurantLatitude)) * cos(radians(areaLatitude)) *
                          cos(radians(areaLongitude)
                              - radians(@currentRestaurantLongitude)) +
...
async function getVisitedInfo(getVisitedInfoParams) {
    ...
    const getVisitedInfoQuery = `
        ...
        select Restaurant.restaurantId,
               concat(userName, '님이 ', restaurantName, '에 방문하였습니다.') as message,
               restaurantName,
               ifnull(firstImageUrl, -1)                             as firstImageUrl,
               case
...
async function checkVisited(visitedInfoParams) {
    ...
    const checkVisitedQuery = `
        ...
        where userId = ?
          and restaurantId = ?
          and timestampdiff(hour, updatedAt, now()) < 23;
...
async function enrollVistied(visitedInfoParams2) {
    ...
    const enrollVistiedQuery = `
        ...
        insert into RestaurantVisited (restaurantId, userId, status)
        VALUES (?, ?, ?);
...
async function getIdFromRestaurantKeyWord(searchWord) {
    ...
    const getIdFromRestaurantKeyWordQuery = `
        ...
        select restaurantId
        from RestaurantKeyWord
        where restaurantKeyWord like concat('%', ?, '%');
        ...
        select EatDeal.eatDealId,
               CASE
                   WHEN TIMESTAMPDIFF(HOUR, EatDeal.updatedAt, now()) > 23
                       THEN IF(TIMESTAMPDIFF(DAY, EatDeal.updatedAt, now()) > 7,
                               date_format(EatDeal.updatedAt, '%Y-%m-%d'),
                 ...
                 left outer join (select EatDealImg.eatDealId, eatDealImgUrl as firstImageUrl
                                  from EatDealImg
                                           inner join (select eatDealId, min(imgId) as firstImageId
                                                      ...
                                                      on EatDealImg.imgId = firstImage.firstImageId
                                  where EatDealImg.status = 1) EatDealImages
                                 on EatDeal.eatDealId = EatDealImages.eatDealId

        where eatDealName like concat('%', ?, '%')
          and EatDeal.status = 1 limit 1;
    `;
